...
const isEthersError = (error: any): error is EthersError => {
  return error != null && typeof error === 'object' && 'reason' in error && 'method' in error && 'transaction' in error
}
...
export const useTransactionFeedback = () => {
  ...
  const reportException = (error: Error | null) => {
    ...
    const description =
      ethersError != null ? `method: ${ethersError.method} errored: ${ethersError.reason}` : 'execution reverted'
    toast({
