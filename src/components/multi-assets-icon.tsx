...
export const MultiAssetsIcons: FC<MultiAssetsIconsProps> = ({ assets, className }) => {
  ...
  return (
    <div className={classNames(containerClasses, width[assets.length], className)}>
      {assets.map((asset, idx) => (
        <Image
          ...
          className={classNames('relative', zIndexes[idx], offsets[idx], {
            'w-[32px] h-[32px]': idx === 0,
            'bg-primary-100 rounded-full ring-4 ring-primary-100 w-[32px] h-[32px]': idx > 0,
          })}
        />
      ))}
    </div>
